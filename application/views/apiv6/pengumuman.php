...
<script type="text/javascript">
    ...
    function searchFilter() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>pengumuman/timeline/<?php echo $uid;?>',
            dataType:'json',
            beforeSend: function () {
                $('#loading_ajax').show();
            },
    ...
    function paginationData(data) {
        ...
        for(emp in data){
            var empRow = '<li><i class="glyphicon glyphicon-envelope"></i>'+
                '<div class="timeline-item">'+
                '<h5 class="timeline-header"><i class="glyphicon glyphicon-calendar"></i> '+data[emp].pesan_tanggal+'</h5>'+
                '<div class="timeline-body" style="padding: 8px;">'+data[emp].pesan_text+'</div>'+
                '<div class="timeline-footer">Diterbitkan oleh '+data[emp].username+'</div>'+
                '</div>'+
                '</li>';
            $('ul.timeline').append(empRow);
        }
